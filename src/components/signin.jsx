...
const[rememberMe, setRememberMe]= useState(false)
  ...
  <div className="flex h-1/3 flex-col items-center">
    <input type="email" placeholder="Email" className="border border-gray-400 w-90 h-10 rounded pl-4 focus:border-pink-500 outline-none" value={email} onChange ={(e)=>setEmail(e.target.value)} />
    <input type="password" placeholder="Password" className="border border-gray-400 w-90 h-10 rounded mt-8 pl-4 focus:border-pink-500 outline-none" value={password}
          onChange={(e) => setPassword(e.target.value)} />
    <div className="flex mt-8 gap-4 items-center">
      <div className="check">
        <input id="check-5" type="checkbox" className="" checked={rememberMe}
  ...
  <div className="flex flex-col h-1/3 items-center justify-center">
    <button className="sign-in-button mt-4 h-8 w-40 rounded-2xl bg-gradient-to-r from-slate-800 to-red-500 w-90 text text-white">
      <a href="./dashboardPage/sidebar-main-page.html">Sign-in</a>
    </button>
    <div className="divider flex items-center mt-2">
      <div className="w-24 h-1 bg-gray mx-2"></div>
      <h1 className="to-register text-gray">Don't have an account yet ?</h1>
      <div className="w-24 h-1 bg-gray mx-2"></div>
    </div>
    <button className="sign-up-button bg-gradient-to-r from-slate-800 to-red-500 mt-4 h-8 w-40 text text-white rounded-2xl"> Sign Up</button>
  </div>
</main>
    </div>
  );
